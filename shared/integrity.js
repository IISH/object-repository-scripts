...
assert(file, "Did not find a file with the query " + query);
assert(file.metadata.pid == pid, "The pid we find " + file.metadata.pid + " is not equal to the expected " + pid);

var count = db.getCollection(ns + '.chunks').count({files_id:file.files_id});
var countExpectedChunks = Math.ceil(length / file.chunkSize);
assert(count == countExpectedChunks, "There are chunks missing !");
