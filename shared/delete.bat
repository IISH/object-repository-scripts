...
if "%action%" == "delete" (
    ...
    call %scripts%\shared\set.bat filesid mongo %db% --quiet --eval "var doc=db.%bucket%.files.findOne({'metadata.pid':'%pid%'}, {_id:1});if ( doc ){print(doc._id)}"
    if not defined filesid (
