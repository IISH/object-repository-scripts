...
if [ "$scope" = "$HOSTNAME" ] || [ "$scope" = "all" ] ; then
    if [ "$1" = "start" ] ; then
        if [ -z "$CYGWIN_HOME" ] ; then
            ...
            CMD="java -server -Dor.properties=$(cygpath --windows $OR) -jar $(cygpath --windows $agent) -id $HOSTNAME -messageQueues $(cygpath --windows $OR_HOME)/pmq-agents-enabled -startup \"\startup.bat\""
        fi
        $CMD > $log 2>&1 &
    else
