...
if [[ $usable -lt 1000 ]] ; then
    python $scripts/utils/sendmail.py --body "$report" --from "$OR_MAIL_FROM" --to "$OR_MAIL_TO" --subject "Storage availabiliy report (units in GiB)" --mail_relay "$OR_MAIL_RELAY" --mail_user "$OR_MAIL_USER" --mail_password "$OR_MAIL_PASSWORD"
fi
